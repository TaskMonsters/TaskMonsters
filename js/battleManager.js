...
class BattleManager {
    ...
    async startBattle(heroData, enemyData) {
        ...
        // Hide special attack gauge and button until Level 7
        ...
        // Show gauge and button only at Level 7+
        if (specialGaugeContainer) {
            specialGaugeContainer.style.display = userLevel >= 7 ? 'block' : 'none';
        }
        if (specialAttackBtn) {
            specialAttackBtn.style.display = userLevel >= 7 ? 'block' : 'none';
        }
        ...
        // Procrastination Ghost effect - enemy misses next attack
        this.enemyWillMiss = false;
        
        // Overthinker Confuse effect - hero damages itself
        this.heroConfusedTurns = 0;
        this.heroConfuseSelfDamage = [10, 20];

        // Set battle background based on level with rotation
        const battleContainer = document.querySelector('.battle-container');
        battleContainer.classList.remove('bg-forest', 'bg-night-town', 'bg-city', 'bg-temple', 'bg-ocean', 'bg-skull-gate', 'bg-space', 'bg-castle', 'bg-city-neon', 'bg-synth-city', 'bg-stone-ruins');
        
        // Check if this is an Overthinker battle - use Stone Ruins arena
        if (this.enemy && this.enemy.isOverthinker) {
            battleContainer.classList.add('bg-stone-ruins');
            console.log('🧠 Overthinker battle - using Stone Ruins arena');
        ...
        } // End of else block for non-Overthinker arenas
    ...
    async playerAttack() {
        ...
        // Process Overthinker Confuse effect - hero damages itself
        if (this.heroConfusedTurns > 0) {
            const minDamage = this.heroConfuseSelfDamage[0];
            const maxDamage = this.heroConfuseSelfDamage[1];
            const selfDamage = Math.floor(Math.random() * (maxDamage - minDamage + 1)) + minDamage;
            
            this.hero.hp = Math.max(0, this.hero.hp - selfDamage);
            this.showHeroDamageAnimation(selfDamage);
            addBattleLog(`💫 Your confused monster hurt itself for ${selfDamage} damage!`);
            this.heroConfusedTurns--;
            
            if (this.heroConfusedTurns === 0) {
                addBattleLog(`✨ Your monster is no longer confused!`);
                // Remove confused visual effect
                const heroSprite = document.getElementById('heroSprite');
                if (heroSprite) {
                    heroSprite.classList.remove('confused-hero');
                }
            } else {
                addBattleLog(`💫 Confusion remaining: ${this.heroConfusedTurns} turn(s)`);
            }
        ...
        // Process Disrupt effect (Disruption Dragon)
        if (this.disruptActive && this.disruptTurns > 0) {
            this.hero.hp = Math.max(0, this.hero.hp - this.disruptDamagePerTurn);
            this.showHeroDamageAnimation(this.disruptDamagePerTurn);
            addBattleLog(`⚡ Disruption dealt ${this.disruptDamagePerTurn} HP damage! (${this.disruptTurns} turns left)`);
            
            this.disruptTurns--;
            if (this.disruptTurns <= 0) {
                this.disruptActive = false;
                addBattleLog('✨ Disruption effect ended!');
                
                // Restore original battle arena background
                const battleArena = document.getElementById('battleArena');
                if (battleArena && this.originalBackground) {
                    battleArena.style.backgroundImage = this.originalBackground;
                }
            }
        ...
        // Requires at least 10 minutes of total focus time to activate
        // Damage scales: 25 base + 1 per 10 minutes (max 35 at 100+ minutes)
        ...
        if (hasFocusAttack) {
            ...
            if (triggerChance <= 0.3) {
                ...
                damage = focusAttackDamage; // Override with focus attack damage
                addBattleLog(`🔥 FOCUS POWER UNLEASHED! Massive ${damage} damage!`);
                console.log(`[🔥 Focus Power] Triggered! Total focus minutes: ${focusMinutes}, Damage: ${damage}`);
            }
        }
    ...
    async playerSpark() {
        ...
        // Calculate damage (18-25 range, melee strike)
        const damage = Math.floor(Math.random() * 8) + 18; // Random between 18-25
        const isDead = this.enemy.takeDamage(damage);
    ...
    async playerFireball() {
        ...
        // Calculate damage (15-20 range as per spec)
        const damage = Math.floor(Math.random() * 6) + 15; // Random between 15-20
        const isDead = this.enemy.takeDamage(damage);
    ...
    async playerPrickler() {
        ...
        // Calculate damage (13-17 range with nuclear explosion)
        const damage = Math.floor(Math.random() * 5) + 13; // Random between 13-17
        const isDead = this.enemy.takeDamage(damage);
    ...
    async playerFreeze() {
        ...
        // Calculate damage (20 damage, freezes for 2 turns)
        const damage = 20;
        const isDead = this.enemy.takeDamage(damage);
        ...
        const enemySprite = document.getElementById('enemySprite');
        if (enemySprite) {
            enemySprite.classList.add('frozen-enemy');
            // Store original animation
            this.originalEnemyAnimation = enemySprite.style.animation;
            // Freeze the animation
            enemySprite.style.animationPlayState = 'paused';
            enemySprite.style.filter = 'brightness(1.3) saturate(0.5) hue-rotate(180deg)';
        }
    ...
    async playerBlueFlame() {
        ...
        // Calculate damage (20-35 range)
        const damage = Math.floor(Math.random() * 16) + 20; // Random between 20-35
        const isDead = this.enemy.takeDamage(damage);
    ...
    async playerProcrastinationGhost() {
        ...
        // Calculate variable damage (30-35 range, enemy misses next attack)
        const damage = Math.floor(Math.random() * 6) + 30; // Random between 30-35
        const isDead = this.enemy.takeDamage(damage);
        
        // Set enemy to miss next attack
        this.enemyWillMiss = true;
        
        // Play enemy hurt animation
        await playEnemyAnimation(this.enemy, 'hurt', 300);
        
        addBattleLog(`👻 Procrastination Ghost dealt ${damage} damage! Enemy will miss next attack!`);
        updateBattleUI(this.hero, this.enemy);
        ...
        if (isDead) {
            ...
            // Enemy will miss their next attack (but still takes turn)
            addBattleLog('👻 Enemy is confused and will miss their next attack!');
            await new Promise(resolve => setTimeout(resolve, 1000));
            await this.enemyTurn();
        }
    }
    ...
    async enemyTurn() {
        ...
        // Check if enemy will miss (from Procrastination Ghost)
        if (this.enemyWillMiss) {
            this.enemyWillMiss = false;
            addBattleLog(`👻 ${this.enemy.name} is confused and misses their attack!`);
            await new Promise(resolve => setTimeout(resolve, 1000));
        ...
        if (this.enemyFrozenTurns > 0) {
            ...
            // If freeze effect ended, restore enemy animation
            if (this.enemyFrozenTurns === 0) {
                const enemySprite = document.getElementById('enemySprite');
                if (enemySprite) {
                    enemySprite.classList.remove('frozen-enemy');
                    enemySprite.style.animationPlayState = 'running';
                    enemySprite.style.filter = '';
                    addBattleLog('❄️ Enemy thawed out!');
                }
            }
        ...
        // Apply Poison Leaf damage at start of enemy turn (15 damage for 4 turns)
        if (this.novaPoisonTurns > 0) {
            const poisonDamage = this.novaPoisonDamage || 15;
        ...
        // Disruption Dragon special attack: Disrupt
        if (this.enemy.hasSpecialAttack && Math.random() < this.enemy.specialAttackChance) {
            await playEnemyAnimation(this.enemy, 'attack1', 600);
            
            // Show bolt projectile
            const enemySprite = document.getElementById('enemySprite');
            const heroSprite = document.getElementById('heroSprite');
            if (this.enemy.projectileSprites && this.enemy.projectileSprites.bolt) {
                await playBoltProjectile(enemySprite, heroSprite, this.enemy.projectileSprites.bolt);
            }
            
            // Apply Disrupt effect
            this.disruptActive = true;
            this.disruptTurns = this.enemy.disruptDuration;
            this.disruptDamagePerTurn = this.enemy.disruptDamagePerTurn;
            
            // Change battle arena background
            const battleArena = document.getElementById('battleArena');
            if (battleArena) {
                this.originalBackground = battleArena.style.backgroundImage;
                battleArena.style.backgroundImage = "url('assets/battle/disruption-background.png')";
                battleArena.style.transition = 'background-image 0.5s ease-in-out';
            }
            
            addBattleLog(`🐉 ${this.enemy.name} used ${this.enemy.specialAttackName}!`);
            addBattleLog(`⚡ Battle arena disrupted! You'll take ${this.disruptDamagePerTurn} HP damage per turn for ${this.disruptTurns} turns!`);
        ...
        // If Disruption Dragon uses normal attack, show bolt projectile
        if (this.enemy.projectileType === 'bolt') {
            const enemySprite = document.getElementById('enemySprite');
            const heroSprite = document.getElementById('heroSprite');
            if (this.enemy.projectileSprites && this.enemy.projectileSprites.bolt) {
                await playBoltProjectile(enemySprite, heroSprite, this.enemy.projectileSprites.bolt);
        ...
        // Overthinker Confuse Effect - causes player monster to damage itself for 2 turns
        if (this.enemy.canConfuse && this.enemy.isOverthinker && Math.random() < this.enemy.confuseChance) {
            this.heroConfusedTurns = this.enemy.confuseDuration || 2;
            this.heroConfuseSelfDamage = this.enemy.confuseSelfDamage || [10, 20];
            addBattleLog(`🧠 The Overthinker confused your monster!`);
            addBattleLog(`💫 Your monster will hurt itself for ${this.heroConfusedTurns} turns!`);
            
            // Visual effect - add confused class to hero
            const heroSprite = document.getElementById('heroSprite');
            if (heroSprite) {
                heroSprite.classList.add('confused-hero');
            }
        }
    ...
    async endBattle(result) {
        ...
        if (result === 'victory') {
            ...
            if (window.gameState) {
                ...
                // Reset consecutive losses on victory
                window.gameState.consecutiveLosses = 0;
            ...
            if (window.gameState) {
                ...
                // Track consecutive losses for egg reversion (only for level 10+)
                if (window.gameState.jerryLevel >= 10 && !window.gameState.isEgg) {
                    window.gameState.consecutiveLosses = (window.gameState.consecutiveLosses || 0) + 1;
                    console.log('[Battle] Consecutive losses:', window.gameState.consecutiveLosses);
                    
                    // Revert to egg after 3 consecutive losses
                    if (window.gameState.consecutiveLosses >= 3) {
                        window.gameState.isEgg = true;
                        window.gameState.health = 0; // Set HP to 0
                        window.gameState.eggRevertedAt = Date.now();
                        window.gameState.consecutiveLosses = 0; // Reset counter
                        console.log('[Battle] Monster reverted to egg after 3 consecutive losses!');
                        
                        // Update sprite to show egg
                        ...
                        setTimeout(() => {
                            if (typeof showSuccessMessage === 'function') {
                                showSuccessMessage('🥚 Reverted to Egg!', 'Gain 50 HP to hatch again!');
                            }
                        }, 2000);
                    }
                }
        ...
        // FIX: Stop battle loop music immediately
        if (window.audioManager) {
        ...
        // Update main UI to reflect correct stats
        ...
        setTimeout(() => {
            ...
            // CRITICAL FIX: Stop ALL music when arena closes
            // This catches cases where user doesn't click Continue
            if (window.audioManager) {
                window.audioManager.stopAllBattleMusic();
                window.audioManager.stopBattleOutcomeMusic();
                console.log('[Battle] All battle music stopped on arena close');
            }
        }, 2000);
    }
    ...
    showDefeatModal(enemyName, xpLost) {
        ...
        okButton.onclick = () => {
            ...
            // CRITICAL FIX: Update UI immediately to show new stats
            if (typeof updateUI === 'function') {
                updateUI();
                console.log('[Defeat Modal] UI updated after battle defeat');
            }
