            ...
            <div class="subtask-item ${subtask.completed ? 'completed' : ''}">
                <label class="subtask-checkbox">
                    <input type="checkbox" 
                           ...
                           onchange="window.subtasksManager.${subtask.completed ? 'uncompleteSubtask' : 'completeSubtask'}(${parentTaskIndex}, '${subtask.id}')">
        ...
        if (task && task.subtasks && task.subtasks.length > 0) {
            ...
            if (incompleteSubtasks.length > 0) {
                const proceed = confirm(
                    `This task has ${incompleteSubtasks.length} incomplete subtask(s). Complete anyway?`
                );
                if (!proceed) return;
            }
        }
