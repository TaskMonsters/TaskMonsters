...
### 2. `/js/battleInit.js`
**Changes:**
- Updated `startTestBattle()` function to check both `window.battleManager` existence AND `initialized` flag
- Added new `maybeTriggerBattle(sourceType)` function
...
### 3. `/index.html`
**Changes:**
- **Regular Task Completion**: Replaced old 30% probability check with `maybeTriggerBattle('regularTask')` for 50% probability
- **Quick Task Completion**: Replaced old 30% probability check with `maybeTriggerBattle('quickTask')` for 20% probability
...
3. **Test Other Actions:**
   - Complete quest tasks, daily challenges, use focus timer
   - Verify NO battles are triggered

4. **Test Battle Mode Toggle:**
   - Disable battle mode in settings
   - Complete tasks
   - Verify battles are properly skipped
